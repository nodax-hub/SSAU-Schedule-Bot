...
from alice.schedule import PairsSet, Week, Day, Pair
...
class SSAUParser:
    ...
    @staticmethod
    def _parse_pairs_set(soup_item: BeautifulSoup, time: tuple[datetime.time, datetime.time]) -> PairsSet:
        pairs_set = []
        for pair in soup_item.find_all("div", class_="schedule__lesson"):
            for pair_type in Pair.PAIR_TYPES:
                ...
                logging.warning(f'[!] Необработанный тип лекции: {pair}')
                continue
